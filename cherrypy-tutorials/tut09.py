...
class StringGeneratorWebService(object):
    ...
    def GET(self):
        with sqlite3.connect(DB_STRING) as c:
            c.execute("SELECT value FROM user_string WHERE session_id=?", [cherrypy.session.id])
        return c.fetchone()
...
def setup_database():
    """
    Create the 'user_string table in the db on server startup
    """
...
def cleanup_database():
    """
    Destroy the 'user_string' table from db on server shutdown:
    """
    with sqlite3.connect(DB_STRING) as con:
